...
class Trader:
    ...
    def aco_orders(
        ...
        if effective_position < 0 and remaining_buy > 0:
            if fair in order_depth.sell_orders:
                ask_volume = -order_depth.sell_orders[fair]
            ...
            if fair in order_depth.buy_orders:
                bid_volume = order_depth.buy_orders[fair]
        ...
        bid_price = min(bid_price, effective_best_ask - 1, fair)
        ask_price = max(ask_price, effective_best_bid + 1, fair)
        ...
        if passive_buy_qty > 0 and bid_price < fair:
            orders.append(Order(product, bid_price, passive_buy_qty))

        if passive_sell_qty > 0 and ask_price > fair:
