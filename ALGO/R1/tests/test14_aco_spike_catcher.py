"""
test14: Fixed-fair ACO + 'spike catcher' passive quotes at fair ± 5, large size.
These fire only on big moves (rare) but give huge edge when they do.
Plus fixed_fair's native tight quote at fair±1.
"""
import json
...
class Trader:
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        # Spike catcher at fair ± SPIKE_OFFSET — only if inventory permits
        deep_bp = fair - cls.SPIKE_OFFSET
        deep_ap = fair + cls.SPIKE_OFFSET
        dbq = min(cls.SPIKE_SIZE, rb)
        dsq = min(cls.SPIKE_SIZE, rs)
