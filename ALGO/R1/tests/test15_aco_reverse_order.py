"""
test15: Fixed-fair ACO, but post MULTIPLE passive quote levels in REVERSE order —
deepest first. Market-trade matching iterates orders sequentially; deepest-first
means trades at deep prices fill at better (deeper) prices for us.
"""
import json
...
class Trader:
    ...
    FAIR = 10000
    TAKE_THRESH = 1
    # Tier sizes (total ≤ limit). Deepest tier first in list.
    BID_TIERS = [(5, 5), (3, 10), (1, 20)]   # (offset, size) — buy at fair - offset
    ASK_TIERS = [(5, 5), (3, 10), (1, 20)]   # sell at fair + offset
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        # Aggressive takes FIRST (priority for best book prices)
        ...
        # Passive quotes: post DEEPEST tier first (deepest offset first in list)
        # So market trades at deep prices get matched at our deep-tier order price
        for offset, sz in cls.BID_TIERS:
            if rb <= 0: break
            q = min(sz, rb)
            if q > 0:
                orders.append(Order(product, fair - offset, q))
                rb -= q
        for offset, sz in cls.ASK_TIERS:
            if rs <= 0: break
            q = min(sz, rs)
            if q > 0:
                orders.append(Order(product, fair + offset, -q))
