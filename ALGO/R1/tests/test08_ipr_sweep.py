"""
test08: IPR aggressive multi-level sweep.
Post buy at best_ask + 10 — this takes ALL asks in book (all 3 levels) immediately,
and the remaining rests as a high-priced passive buy that bots will readily hit.
Theory: speedup to max long worth small extra slippage.
"""
import json
    ...
    def run(self, state: TradingState):
        ...
        for product, od in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                buy_need = limit - pos
                if buy_need > 0:
                    orders.append(Order(product, ba + self.IPR_SWEEP_OFFSET, buy_need))
