"""
test06: IPR scalp: stay net long 80, but post passive sell of SCALP_SIZE inside
the spread to catch aggressor buys (at local spikes). After a fill we're <80
and the next tick we buy aggressively to refill.
ACO: unchanged fixed_fair baseline.
"""
import json
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product, od in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                orders = self.ipr(product, od, bb, ba, pos, limit)
    ...
    @classmethod
    def ipr(cls, product, od, bb, ba, pos, limit):
        orders = []
        # Always refill long up to 80 (aggressive on best_ask)
        buy_need = limit - pos
        if buy_need > 0:
            orders.append(Order(product, ba, buy_need))
        # Scalp: post passive sell inside spread for SCALP_SIZE (up to pos)
        # This only fires once we're long
        if pos >= cls.IPR_SCALP_SIZE and ba - bb >= 2:
            scalp_price = ba - 1
            orders.append(Order(product, scalp_price, -cls.IPR_SCALP_SIZE))
        return orders
