"""
test25: Ladder + imbalance skew. Center ladder at fair+imb*skew.
"""
import json
...
class Trader:
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        # Skew ladder center based on imbalance. Stay near fair.
        center = fair + round(imb * 1.0)
        ...
        for offset, sz in cls.BID_LADDER:
            ...
            q = min(sz, rb)
            price = center - offset
            if price >= fair: continue  # don't post at or above fair
            if q > 0: orders.append(Order(product, price, q)); rb -= q
        for offset, sz in cls.ASK_LADDER:
            if rs <= 0: break
            q = min(sz, rs)
            price = center + offset
            if price <= fair: continue
            if q > 0: orders.append(Order(product, price, -q)); rs -= q
        return orders
