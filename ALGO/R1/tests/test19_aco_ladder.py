"""
test19: ACO LADDER. Post passive at every price level 9990-9999 and 10001-10010.
KEY: market trades are distributed across the whole below/above-fair range.
Each passive order fills AT ITS OWN PRICE — so a bid at 9992 that catches a
market trade at 9985 fills at 9992 (8 tick edge vs fair).
Order processing: post DEEPEST-FIRST so a trade at 9985 matches the 9990 level
first (edge 10) before shallower levels.
"""
import json
...
class Trader:
    ...
    FAIR = 10000
    LADDER_DEPTH = 10   # levels 9990..9999 (and 10001..10010)
    SIZE_PER_LEVEL = 5  # total = 50 per side; safe under pos limit
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        # LADDER PASSIVE QUOTES: deepest first
        # Buy ladder: fair-10, fair-9, ..., fair-1 (deepest first)
        for offset in range(cls.LADDER_DEPTH, 0, -1):
            if rb <= 0: break
            price = fair - offset
            q = min(cls.SIZE_PER_LEVEL, rb)
            if q > 0:
                orders.append(Order(product, price, q))
                rb -= q
        # Sell ladder: fair+10, fair+9, ..., fair+1 (deepest first)
        for offset in range(cls.LADDER_DEPTH, 0, -1):
            if rs <= 0: break
            price = fair + offset
            q = min(cls.SIZE_PER_LEVEL, rs)
            if q > 0:
                orders.append(Order(product, price, -q))
                rs -= q
        return orders
