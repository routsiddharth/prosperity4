"""
test09: ACO focused on faster turnover.
- Pin passive quotes at 9999/10001 (no skew).
- Size 30 instead of 15.
- When |pos| > 40, also take at fair (breakeven) to rotate.
- Aggressive flatten at fair.
"""
import json
...
class Trader:
    ...
    ACO_FAIR = 10000
    QUOTE_SIZE = 30
    SOFT_POS = 40  # above this, also flatten at fair
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        # Flatten at fair (breakeven) ONLY if heavily inventoried — to rotate
        if eff < -cls.SOFT_POS and rb > 0 and fair in od.sell_orders:
            q = min(rb, -od.sell_orders[fair], abs(eff) - cls.SOFT_POS // 2)
            if q > 0: orders.append(Order(product, fair, q)); rb -= q; eff += q
        elif eff > cls.SOFT_POS and rs > 0 and fair in od.buy_orders:
            q = min(rs, od.buy_orders[fair], abs(eff) - cls.SOFT_POS // 2)
            if q > 0: orders.append(Order(product, fair, -q)); rs -= q; eff -= q

        # Passive quotes: fixed at 9999/10001, size 30
        pbq = min(cls.QUOTE_SIZE, rb)
        psq = min(cls.QUOTE_SIZE, rs)
        # Inventory-aware: smaller on the over-extended side
        if eff > 20:
            pbq = max(0, pbq - (eff - 20))
        elif eff < -20:
            psq = max(0, psq - (abs(eff) - 20))

        if pbq > 0: orders.append(Order(product, fair - 1, pbq))
        if psq > 0: orders.append(Order(product, fair + 1, -psq))
        return orders
