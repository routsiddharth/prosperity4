"""
test16: IPR trend-anchored scalp.
- IPR known drift ≈ +0.1/tick within a day.
- Compute trend_fair using EMA of mid + small correction.
- Stay long 80 normally.
- When mid > trend_fair + OVERSHOOT, sell SCALP_SIZE at mid-1 (catch spike).
- When that fill happens, rebuy at best_ask next tick to restore.
- Key: only sell when mid is SIGNIFICANTLY above trend.
ACO: fixed_fair baseline.
"""
import json
...
class Trader:
    ...
    QUOTE_SIZE = 15
    IPR_EMA_ALPHA = 0.1
    IPR_TREND_PER_TICK = 0.01  # +0.1 per 10 tick per data analysis
    IPR_OVERSHOOT = 4
    IPR_SCALP_SIZE = 20
    ...
    def run(self, state: TradingState):
        ...
        for product, od in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ema = data.get("ipr_ema", mid)
                ema = self.IPR_EMA_ALPHA * mid + (1 - self.IPR_EMA_ALPHA) * ema
                data["ipr_ema"] = ema
                # Trend fair: EMA (captures trend implicitly with fast alpha)
                trend_fair = ema
                orders = self.ipr(product, od, bb, ba, pos, limit, mid, trend_fair)
    ...
    @classmethod
    def ipr(cls, product, od, bb, ba, pos, limit, mid, trend_fair):
        orders = []
        # Always keep filling to max long (using best_ask)
        buy_need = limit - pos
        if buy_need > 0:
            orders.append(Order(product, ba, buy_need))

        # Scalp: if mid is notably above trend AND we have inventory, sell a chunk
        if pos >= cls.IPR_SCALP_SIZE and mid > trend_fair + cls.IPR_OVERSHOOT:
            sell_price = int(mid) - 1   # aggressive: sell right at current bid-1ish
            sell_size = min(cls.IPR_SCALP_SIZE, pos)
