"""
test11: Fixed-fair ACO + asymmetric tier-2 passive quote.
When position is long: post extra sell at fair+2. When short: extra buy at fair-2.
Goal: unwind faster without accumulating more on the same side.
"""
import json
...
class Trader:
    ...
    QUOTE_SIZE = 15
    TIER2_SIZE = 20
    TIER2_POS_THRESH = 10  # only post tier-2 if |pos| > this
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        if pbq > 0 and bp < sfair: orders.append(Order(product, bp, pbq)); rb -= pbq
        if psq > 0 and ap > sfair: orders.append(Order(product, ap, -psq)); rs -= psq

        # Asymmetric tier-2: unwind-direction only
        if eff > cls.TIER2_POS_THRESH:  # long → post extra sell at fair+2
            q = min(cls.TIER2_SIZE, rs)
            if q > 0: orders.append(Order(product, fair + 2, -q))
        elif eff < -cls.TIER2_POS_THRESH:  # short → post extra buy at fair-2
            q = min(cls.TIER2_SIZE, rb)
            if q > 0: orders.append(Order(product, fair - 2, q))
        return orders
