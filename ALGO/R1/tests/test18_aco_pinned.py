"""
test18: ACO — pin passive quote at fair±1 ALWAYS.
Theory: passive buy at 9999 captures all aggressor-sells at ≤9999 at OUR price 9999,
which is always 1-tick edge. Penny-jumping inside a wider spread gives less.
"""
import json
    ...
    def run(self, state: TradingState):
        ...
        for product, od in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                if eff < 0 and rb > 0 and self.FAIR in od.sell_orders:
                    q = min(rb, -od.sell_orders[self.FAIR], abs(eff))
                    if q > 0: orders.append(Order(product, self.FAIR, q)); rb -= q; eff += q
                elif eff > 0 and rs > 0 and self.FAIR in od.buy_orders:
                    q = min(rs, od.buy_orders[self.FAIR], abs(eff))
                    if q > 0: orders.append(Order(product, self.FAIR, -q)); rs -= q; eff -= q
                # Pin passive at fair±1
                pbq = min(self.QUOTE_SIZE, rb); psq = min(self.QUOTE_SIZE, rs)
                if pbq > 0: orders.append(Order(product, self.FAIR - 1, pbq))
                if psq > 0: orders.append(Order(product, self.FAIR + 1, -psq))
            result[product] = orders
