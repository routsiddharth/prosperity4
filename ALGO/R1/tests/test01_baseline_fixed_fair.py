"""
test01: Exact clone of fixed_fair/207376.py.
Baseline reference. ACO: fixed fair=10000, imbalance skew, passive fair±1 quotes.
IPR: buy best_ask aggressively to max long.
Expected: ~282k.
"""
import json
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product, od in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                orders = self.ipr(product, ba, pos, limit)
    ...
    @staticmethod
    def ipr(product, best_ask, pos, limit):
        buy_qty = limit - pos
        return [Order(product, best_ask, buy_qty)] if buy_qty > 0 else []
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        # effective book top after our takes
        ebb, eba = bb, ba
        skew_fair = int(fair + imb * 1.5)
        if skew_fair >= ba:
            for p in sorted(od.sell_orders):
                vol = -od.sell_orders[p]; taken = sum(o.quantity for o in orders if o.quantity > 0 and o.price == p)
                if taken < vol: eba = p; break
            else: eba = skew_fair + 1
        if skew_fair <= bb:
            for p in sorted(od.buy_orders, reverse=True):
                vol = od.buy_orders[p]; taken = sum(-o.quantity for o in orders if o.quantity < 0 and o.price == p)
                if taken < vol: ebb = p; break
            else: ebb = skew_fair - 1

        pbq = min(cls.ACO_QUOTE_SIZE, rb); psq = min(cls.ACO_QUOTE_SIZE, rs)
        spread = eba - ebb
        if spread >= 2: bp, ap = ebb + 1, eba - 1
        else: bp, ap = ebb, eba
        bp = min(bp, eba - 1, skew_fair); ap = max(ap, ebb + 1, skew_fair)
        if bp >= ap: bp, ap = skew_fair - 1, skew_fair + 1

        if pbq > 0 and bp < skew_fair: orders.append(Order(product, bp, pbq))
        if psq > 0 and ap > skew_fair: orders.append(Order(product, ap, -psq))
        return orders
