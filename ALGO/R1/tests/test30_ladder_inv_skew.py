"""
test30: Ladder + flatten + inventory-aware size skew.
When long, bid-side size smaller, ask-side bigger (to unwind).
"""
import json
...
class Trader:
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        if pos < 0 and fair in od.sell_orders:
            q = min(rb, -od.sell_orders[fair], abs(pos))
            if q > 0: orders.append(Order(product, fair, q)); rb -= q
        elif pos > 0 and fair in od.buy_orders:
            q = min(rs, od.buy_orders[fair], abs(pos))
            if q > 0: orders.append(Order(product, fair, -q)); rs -= q

        # Size skew based on position (0 at pos=0, max 3 at |pos|=80)
        skew = round(abs(pos) / limit * 3)
        # If long: shrink bid size, enlarge ask size; vice versa when short
        bid_sz = max(1, cls.BASE_SIZE - skew) if pos >= 0 else cls.BASE_SIZE + skew
        ask_sz = cls.BASE_SIZE + skew if pos >= 0 else max(1, cls.BASE_SIZE - skew)

        for offset in range(cls.DEPTH, 0, -1):
            if rb <= 0: break
            q = min(bid_sz, rb)
