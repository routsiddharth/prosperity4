"""
test02: ACO multi-level quotes. Same IPR + take/flatten logic.
Adds passive quotes at fair±2 and fair±3 to catch fills when market drifts.
"""
import json
...
class Trader:
    ...
    # Multi-level sizes at fair±1, fair±2, fair±3
    ACO_L1_SIZE = 20
    ACO_L2_SIZE = 20
    ACO_L3_SIZE = 20
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        # Multi-level passive quotes: buy at skew_fair-1, -2, -3 and sell at skew_fair+1, +2, +3.
        # Each level has its own size; total bounded by capacity.
        bid_sizes = [(skew_fair - 1, cls.ACO_L1_SIZE), (skew_fair - 2, cls.ACO_L2_SIZE), (skew_fair - 3, cls.ACO_L3_SIZE)]
        ask_sizes = [(skew_fair + 1, cls.ACO_L1_SIZE), (skew_fair + 2, cls.ACO_L2_SIZE), (skew_fair + 3, cls.ACO_L3_SIZE)]

        for price, sz in bid_sizes:
            if rb <= 0: break
            q = min(sz, rb)
            if q > 0 and price < fair:
                orders.append(Order(product, price, q))
                rb -= q
        for price, sz in ask_sizes:
            if rs <= 0: break
            q = min(sz, rs)
            if q > 0 and price > fair:
                orders.append(Order(product, price, -q))
