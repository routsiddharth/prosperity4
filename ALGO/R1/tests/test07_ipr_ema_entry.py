"""
test07: IPR EMA-based entry.
- Maintain EMA of mid.
- Aggressive take at any ask <= EMA - 1 (buying the dip).
- Passive penny-jump bid at best_bid+1 for the rest.
- Never sell.
ACO: unchanged fixed_fair.
"""
import json
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product, od in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                data["ipr_ema"] = ema
                orders = self.ipr(product, od, bb, ba, pos, limit, ema)
    ...
    @classmethod
    def ipr(cls, product, od, bb, ba, pos, limit, ema):
        orders = []
        rb = limit - pos
        if rb <= 0: return orders

        # Take any ask at or below ema (buy dips)
        for p in sorted(od.sell_orders):
            if p <= int(ema) and rb > 0:
                q = min(rb, -od.sell_orders[p])
                if q > 0: orders.append(Order(product, p, q)); rb -= q
            else: break

        # Passive penny-jump bid for remaining capacity
        if rb > 0:
            orders.append(Order(product, bb + 1, rb))
        return orders
