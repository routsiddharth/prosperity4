"""
test05: Fixed-fair ACO + deeper unwind logic. After our passive quote, also
post a 'relief' quote several ticks wider at big size. These rarely fill but
when they do (large move), they give huge edge AND unload inventory quickly.
"""
import json
...
class Trader:
    ...
    TAKE_THRESH = 1
    TIGHT_SIZE = 15
    DEEP_OFFSET = 4       # post deeper tier at fair±4
    DEEP_SIZE = 25
    ...
    def aco(cls, product, od, bb, ba, pos, limit):
        ...
        # Tight passive quote at fair±1 (primary edge)
        sfair = int(fair + imb * 1.5)
        tight_bp = min(sfair - 1, bb + 1) if ba - bb >= 2 else bb
        tight_ap = max(sfair + 1, ba - 1) if ba - bb >= 2 else ba
        tight_bp = min(tight_bp, fair - 1)
        tight_ap = max(tight_ap, fair + 1)
        if tight_bp >= tight_ap:
            tight_bp, tight_ap = fair - 1, fair + 1

        pbq = min(cls.TIGHT_SIZE, rb); psq = min(cls.TIGHT_SIZE, rs)
        if pbq > 0: orders.append(Order(product, tight_bp, pbq)); rb -= pbq
        if psq > 0: orders.append(Order(product, tight_ap, -psq)); rs -= psq

        # Deep tier at fair±4 — catches large moves for big edge
        deep_bp = fair - cls.DEEP_OFFSET
        deep_ap = fair + cls.DEEP_OFFSET
        dbq = min(cls.DEEP_SIZE, rb); dsq = min(cls.DEEP_SIZE, rs)
        if dbq > 0: orders.append(Order(product, deep_bp, dbq))
        if dsq > 0: orders.append(Order(product, deep_ap, -dsq))
        return orders
