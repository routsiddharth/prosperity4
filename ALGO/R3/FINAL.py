...
class Trader:
    ...
    # =====================================================
    # HYDROGEL_PACK — microprice ladder MM
    # =====================================================
    HYDROGEL = "HYDROGEL_PACK"
    HYDROGEL_BASE_FAIR = 9992
    HYDROGEL_MICRO_W = 0.7
    HYDROGEL_LADDER_TICKS = (2, 5, 8)
    HYDROGEL_LADDER_SIZE = 12
    HYDROGEL_INV_SKEW = 22
    HYDROGEL_INV_QUAD = 12
    HYDROGEL_TAKE_WIDTH = 8
    HYDROGEL_TAKE_SIZE = 25

    # =====================================================
    # VELVETFRUIT_EXTRACT — microprice ladder MM
    # =====================================================
    EXTRACT = "VELVETFRUIT_EXTRACT"
    EXTRACT_BASE_FAIR = 5250
    EXTRACT_MICRO_W = 0.7
    EXTRACT_LADDER_TICKS = (2, 5, 8)
    EXTRACT_LADDER_SIZE = 10
    EXTRACT_INV_SKEW = 12
    ...
    def run(self, state: TradingState):
        ...
        if self.halted.get(self.EXTRACT):
            for opt in (self.VEV4000, self.OPTION, self.V5400, self.V6000, self.V6500):
    ...
    # =====================================================
    # Microprice ladder market maker (HYDROGEL & EXTRACT)
    # =====================================================
    def microprice(self, order_depth):
        ...
        best_bid = max(order_depth.buy_orders.keys())
        best_ask = min(order_depth.sell_orders.keys())
        return (best_ask * total_bid + best_bid * total_ask) / (total_bid + total_ask)

    def _ladder_trade(self, product, order_depth, position, base_fair, micro_w,
                      ladder_ticks, ladder_size, inv_skew, inv_quad,
                      take_w, take_size):
        orders = []
        best_bid = max(order_depth.buy_orders.keys())
        best_ask = min(order_depth.sell_orders.keys())
        ...
        fair = micro_w * micro + (1 - micro_w) * base_fair
        fair -= position_skew * inv_skew
        if inv_quad:
            fair -= inv_quad * (position_skew ** 3)
        ...
        for ask_price, ask_vol_raw in sorted(order_depth.sell_orders.items()):
            ...
            if ask_price <= fair - take_w:
                qty = min(take_size, ask_vol, buy_cap)
                if qty > 0:
        ...
        for bid_price, bid_vol in sorted(order_depth.buy_orders.items(), reverse=True):
            ...
            if bid_price >= fair + take_w:
                qty = min(take_size, bid_vol, sell_cap)
                if qty > 0:
        ...
        for tick in ladder_ticks:
            if buy_cap <= 0:
                break
            bid_q = math.floor(fair - tick)
            bs = min(ladder_size, buy_cap)
            if bs > 0 and bid_q < best_ask and bid_q > 0:
                orders.append(Order(product, bid_q, bs))
                buy_cap -= bs
        for tick in ladder_ticks:
            if sell_cap <= 0:
                break
            ask_q = math.ceil(fair + tick)
            ss = min(ladder_size, sell_cap)
            if ss > 0 and ask_q > best_bid:
                orders.append(Order(product, ask_q, -ss))
                sell_cap -= ss
        return orders

    def trade_hydrogel(self, product, order_depth, position):
        return self._ladder_trade(
            product, order_depth, position,
            self.HYDROGEL_BASE_FAIR, self.HYDROGEL_MICRO_W,
            self.HYDROGEL_LADDER_TICKS, self.HYDROGEL_LADDER_SIZE,
            self.HYDROGEL_INV_SKEW, self.HYDROGEL_INV_QUAD,
            self.HYDROGEL_TAKE_WIDTH, self.HYDROGEL_TAKE_SIZE,
        )
    ...
    def trade_extract(self, product, order_depth, position):
        return self._ladder_trade(
            product, order_depth, position,
            self.EXTRACT_BASE_FAIR, self.EXTRACT_MICRO_W,
            self.EXTRACT_LADDER_TICKS, self.EXTRACT_LADDER_SIZE,
            self.EXTRACT_INV_SKEW, 0,
            self.EXTRACT_TAKE_WIDTH, self.EXTRACT_TAKE_SIZE,
        )
