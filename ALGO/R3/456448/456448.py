    ...
    def run(self, state: TradingState):
        ...
        if self.EXTRACT in state.order_depths:
            ...
            if depth.buy_orders and depth.sell_orders:
                ...
                underlying_mid = self.get_mid(depth)
                imbalance = self.get_imbalance(depth)

        # -------------------------
        # VEV_4000 market making
        # -------------------------
        if self.VEV4000 in state.order_depths and underlying_mid is not None:
            depth_4000 = state.order_depths[self.VEV4000]

            if depth_4000.buy_orders and depth_4000.sell_orders:
                position_4000 = int(state.position.get(self.VEV4000, 0))

                orders_by_product[self.VEV4000] = self.trade_4000(
                    depth_4000,
                    position_4000,
                    underlying_mid,
                )
        ...
        # VEV_5300 short premium
        # -------------------------
        if self.OPTION in state.order_depths and underlying_mid is not None:
        ...
        # -------------------------
        if self.V5400 in state.order_depths and underlying_mid is not None:
            v5400_depth = state.order_depths[self.V5400]

            if v5400_depth.buy_orders and v5400_depth.sell_orders:
