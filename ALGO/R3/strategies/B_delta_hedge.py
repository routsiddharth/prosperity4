...
class Trader:
    ...
    def _run(
        ...
        # Strategy B: replace VELVETFRUIT MM orders with a delta-hedge order
        # based on projected post-tick voucher positions. All VELVETFRUIT
        # capacity is devoted to hedging voucher delta — no MM on the
        # underlying.
        ...
        if velv_depth and velv_depth.buy_orders and velv_depth.sell_orders:
            ...
            T = 0.5
            portfolio_delta = 0.0
            for sym, orders_list in result.items():
                K = _voucher_strike(sym)
                if K is None:
                    continue
                projected_pos = state.position.get(sym, 0) + sum(o.quantity for o in orders_list)
                d = _bs_delta(S, K, T, SIGMA)
                portfolio_delta += projected_pos * d

            target_velv = -int(round(portfolio_delta))
            velv_pos = state.position.get("VELVETFRUIT_EXTRACT", 0)
            velv_limit = self.POSITION_LIMITS.get("VELVETFRUIT_EXTRACT", 200)
            target_velv = max(-velv_limit, min(velv_limit, target_velv))
            need = target_velv - velv_pos
            velv_orders = []
            if need > 0:
                # Buy up via best asks
                remaining = need
                for p in sorted(velv_depth.sell_orders.keys()):
                    if remaining <= 0:
                        break
                    v = -velv_depth.sell_orders[p]
                    q = min(remaining, v)
                    if q > 0:
                        velv_orders.append(Order("VELVETFRUIT_EXTRACT", p, q))
                        remaining -= q
            elif need < 0:
                remaining = -need
                for p in sorted(velv_depth.buy_orders.keys(), reverse=True):
                    if remaining <= 0:
                        break
                    v = velv_depth.buy_orders[p]
                    q = min(remaining, v)
                    if q > 0:
                        velv_orders.append(Order("VELVETFRUIT_EXTRACT", p, -q))
                        remaining -= q
            result["VELVETFRUIT_EXTRACT"] = self._clamp_to_position_limit(
                velv_orders, velv_pos, velv_limit
            )
