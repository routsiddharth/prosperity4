...
class Trader:
    ...
    # Strategy D: residual-trading parameters
    STATARB_Z_THRESHOLD = 1.5   # std-devs past predicted to lean
    STATARB_SHIFT = 2           # price-tick bias added/subtracted from anchor
    ...
    def _run(
        ...
        resid_ema: Dict[str, float] = dict(saved.get("resid_ema", {}))
        resid_var: Dict[str, float] = dict(saved.get("resid_var", {}))

        # Build basket: mean voucher mid (weighted by volume inverse)
        voucher_mids: Dict[str, float] = {}
        for sym, depth in state.order_depths.items():
            if _voucher_strike(sym) is None or sym not in self.ACTIVE_PRODUCTS:
                continue
            if not depth.buy_orders or not depth.sell_orders:
                continue
            bb = max(depth.buy_orders.keys())
            ba = min(depth.sell_orders.keys())
            voucher_mids[sym] = (bb + ba) / 2.0

        # Residual = voucher_mid minus (predicted from VELVETFRUIT mid via
        # strike-specific beta, using a running mean as the simplest regression)
        ...
        anchor_shift: Dict[str, float] = {}
        if S is not None:
            for sym, m in voucher_mids.items():
                K = _voucher_strike(sym)
                # Intrinsic-based expected value: intrinsic(S,K) + TV_ema(sym)
                intrinsic = max(S - K, 0.0)
                tv = m - intrinsic
                tv_ema_prev = resid_ema.get(sym)
                tv_var_prev = resid_var.get(sym, 1.0)
                alpha_r = 0.1
                if tv_ema_prev is None:
                    tv_ema_new = tv
                    tv_var_new = tv_var_prev
                else:
                    tv_ema_new = alpha_r * tv + (1 - alpha_r) * tv_ema_prev
                    tv_var_new = alpha_r * (tv - tv_ema_prev) ** 2 + (1 - alpha_r) * tv_var_prev
                resid_ema[sym] = tv_ema_new
                resid_var[sym] = tv_var_new
                std = math.sqrt(max(tv_var_prev, 1e-6))
                z = (tv - tv_ema_new) / std if std > 0 else 0.0
                # If z > threshold, mid is rich -> bias anchor DOWN so we sell more
                # If z < -threshold, mid is cheap -> bias anchor UP so we buy more
                if z > self.STATARB_Z_THRESHOLD:
                    anchor_shift[sym] = -self.STATARB_SHIFT
                elif z < -self.STATARB_Z_THRESHOLD:
                    anchor_shift[sym] = +self.STATARB_SHIFT
        ...
        for product, order_depth in state.order_depths.items():
            ...
            try:
                ...
                # Strategy D: shift anchor by residual-based bias
                if product in anchor_shift:
                    anchor = anchor + anchor_shift[product]
