...
Merges 7 distinct product-family strategies (each previously a standalone
submission) under one Trader class. Each sub-strategy keeps its original logic
verbatim; the master Trader namespaces their traderData blobs under JSON keys
so per-strategy state survives across ticks without collisions.

Sub-strategies:
  microchip   <- 572216 (4-sleeve mean-reversion: basket3, basket4, tri idio, rect MR)
  translator  <- 563404 (passive 5-product market maker)
  snackpack   <- 560678 (two-basket stat arb on SNACKPACK_*)
  galaxy      <- 557223 (intraday schedule + RV + pair-spread on GALAXY_SOUNDS_*)
  panel       <- 559020 (selected-legs basket reversion on PANEL_* and UV_VISOR_*)
  sleeppod    <- 566677 (intraday schedule + RV + pair-spread on SLEEP_POD_*)
"""
...
# =========================================================================
# MICROCHIP — from 572216 (4-sleeve mean reversion)
# =========================================================================
...
def _mc_sleeve_legs(sid: str) -> Dict[str, int]:
    cfg = PARAMS[sid]
    if cfg["kind"] == "basket":
        return cfg["legs"]
    return {cfg["target"]: +1}
...
def _mc_trim(buf: list) -> list:
    if len(buf) > MAX_BUFFER:
        return buf[-MAX_BUFFER:]
    return buf


def _mc_compute_z(series: List[float], window: int, ddof: int = 1):
    n = len(series)
    if n < window:
        return None
    s = series[-window:]
...
def _mc_compute_basket_z(state: dict, legs: Dict[str, int], window: int):
    ...
    for i in range(start, n):
        ...
        for c, sgn in legs.items():
            v += (mids[c][i] * 0.5) * sgn
        series.append(v)
    return _mc_compute_z(series, window)


def _mc_compute_single_z(state: dict, target: str, window: int):
    n = len(state["mids_x2"][target])
    if n < window:
        return None
    seg = state["mids_x2"][target][-window:]
    return _mc_compute_z([v * 0.5 for v in seg], window)


def _mc_compute_idio_z(state: dict, target: str, window: int):
    ...
    return _mc_compute_z(cum, window)


def _mc_z_for_window(sid: str, state: dict, window: int):
    cfg = PARAMS[sid]
    if cfg["kind"] == "basket":
        return _mc_compute_basket_z(state, cfg["legs"], window)
    if cfg["kind"] == "single":
        return _mc_compute_single_z(state, cfg["target"], window)
    if cfg["kind"] == "idio":
        return _mc_compute_idio_z(state, cfg["target"], window)
    return None


def _mc_compute_z_for_sleeve(sid: str, state: dict, tick: int):
    cfg = PARAMS[sid]
    z_long = _mc_z_for_window(sid, state, cfg["window"])
    if z_long is not None:
        return z_long, "long"
    if cfg.get("warmup_mode") == "dual":
        w_short = cfg.get("w_short", max(100, cfg["window"] // 5))
        z_short = _mc_z_for_window(sid, state, w_short)
        if z_short is not None:
            return z_short, "short"
    return None, None


def _mc_proposed_entry(sid: str, units_now: int, z_now, regime):
    cfg = PARAMS[sid]
...
def _mc_should_exit(sid: str, st: dict, z_now, regime, tick: int) -> bool:
    cfg = PARAMS[sid]
    units = st["units"]
...
def _mc_current_total_legs(sleeves_state: dict) -> Dict[str, int]:
    total = {c: 0 for c in CHIPS}
    for sid, st in sleeves_state.items():
        if st["units"] != 0:
            for c, sgn in _mc_sleeve_legs(sid).items():
                total[c] += st["units"] * sgn
    return total


def _mc_fits(total: Dict[str, int], leg_vec: Dict[str, int]) -> bool:
...
def _mc_cap_units_to_fit(units_desired: int,
    ...
    for u in range(abs(int(units_desired)), 0, -1):
        ...
        if _mc_fits(total, leg_vec):
            return sgn * u
    return 0


def _mc_mode1_priority_partial(desired, current_total):
    accepted = []
    total = dict(current_total)
    for sid, units in sorted(desired, key=lambda x: PARAMS[x[0]]["priority"]):
        u = _mc_cap_units_to_fit(units, _mc_sleeve_legs(sid), total)
        if u != 0:
            for c, sg in _mc_sleeve_legs(sid).items():
    ...
    def run(self, state: TradingState):
        ...
        for c in CHIPS:
            if mids_now[c] is not None:
                ...
                tstate["mids_x2"][c] = _mc_trim(tstate["mids_x2"][c])
        ...
        pre_intent = _mc_current_total_legs(sleeves_st)
        chips_with_pending = set()
        for c in CHIPS:
            if pre_intent[c] != state.position.get(c, 0):
                chips_with_pending.add(c)

        z_by_sleeve = {sid: _mc_compute_z_for_sleeve(sid, tstate, tick) for sid in SLEEVE_ORDER}
        ...
        for sid in SLEEVE_ORDER:
            ...
            exit_now = eod or _mc_should_exit(sid, st, z, regime, tick)
            if exit_now:
                u = st["units"]
                ...
                if st["entry_price"] is not None:
                    ...
                    cost = abs(u) * sum(SPREAD_MED[c] for c in _mc_sleeve_legs(sid))
        ...
        if not eod and not tstate["halted"]:
            ...
            for sid in SLEEVE_ORDER:
                ...
                if st.get("cooldown_until", -1) > tick:
                    continue
                z, regime = z_by_sleeve[sid]
                u = _mc_proposed_entry(sid, st["units"], z, regime)
                if u == 0:
                    continue
                legs = _mc_sleeve_legs(sid)
                ok = True
                for c in legs:
                    sp = spread_now.get(c)
            ...
            existing_total = _mc_current_total_legs(sleeves_st)
            accepted = _mc_mode1_priority_partial(desired, existing_total)
        ...
        final_intent = _mc_current_total_legs(sleeves_st)
        for c in CHIPS:
...
# =========================================================================
# TRANSLATOR — from 563404 (passive 5-product market maker)
# =========================================================================
...
# =========================================================================
# SNACKPACK — from 560678 (two-basket stat arb)
# =========================================================================
...
class SnackpackTrader:
        ...
        if all(p in mids for p in self.BASKET_A_LEGS):
            ...
            if z_a is not None:
                self.manage_basket_a_position(
                    ...
                    microprices=microprices, imbalances=imbalances, z=z_a,
                )
        ...
        if all(p in mids for p in self.BASKET_B_LEGS):
            ...
            if z_b is not None:
                self.manage_basket_b_position(
                    state=state, orders=orders,
                    best_bids=best_bids, best_asks=best_asks,
                    microprices=microprices, imbalances=imbalances, z=z_b,
                )
    ...
    def manage_basket_a_position(self, state, orders, best_bids, best_asks,
                                  microprices, imbalances, z):
        ...
        if z > self.ENTRY_Z:
            self.open_short_basket_a(state, orders, best_bids, best_asks,
                                      microprices, imbalances, base_size)
        elif z < -self.ENTRY_Z:
            self.open_long_basket_a(state, orders, best_bids, best_asks,
                                     microprices, imbalances, base_size)

    def manage_basket_b_position(self, state, orders, best_bids, best_asks,
                                  microprices, imbalances, z):
        if abs(z) <= self.EXIT_Z:
        ...
        if abs(z) > self.PANIC_Z:
            ...
            base_size = min(self.STAT_ARB_MAX_SIZE,
                            max(1, int(self.STAT_ARB_BASE_SIZE * abs(z) / self.ENTRY_Z)))
        if z > self.ENTRY_Z:
            self.open_short_basket_b(state, orders, best_bids, best_asks,
                                      microprices, imbalances, base_size)
        elif z < -self.ENTRY_Z:
            self.open_long_basket_b(state, orders, best_bids, best_asks,
                                     microprices, imbalances, base_size)

    def open_short_basket_a(self, state, orders, best_bids, best_asks,
                            microprices, imbalances, base_size):
        for product, sign in self.BASKET_A_LEGS.items():
            self._send_leg_order(product, -sign, state, orders, best_bids,
                                  best_asks, microprices, imbalances, base_size)

    def open_long_basket_a(self, state, orders, best_bids, best_asks,
                           microprices, imbalances, base_size):
        for product, sign in self.BASKET_A_LEGS.items():
            self._send_leg_order(product, +sign, state, orders, best_bids,
                                  best_asks, microprices, imbalances, base_size)
    ...
    def open_short_basket_b(self, state, orders, best_bids, best_asks,
                            microprices, imbalances, base_size):
        for product, sign in self.BASKET_B_LEGS.items():
            self._send_leg_order(product, -sign, state, orders, best_bids,
                                  best_asks, microprices, imbalances, base_size)

    def open_long_basket_b(self, state, orders, best_bids, best_asks,
                           microprices, imbalances, base_size):
        for product, sign in self.BASKET_B_LEGS.items():
            self._send_leg_order(product, +sign, state, orders, best_bids,
                                  best_asks, microprices, imbalances, base_size)
    ...
    def _send_leg_order(self, product, direction, state, orders, best_bids,
                         best_asks, microprices, imbalances, base_size):
    ...
    def micro_signal(self, mid, microprice, imbalance) -> int:
        signal = 0
        if microprice > mid:
            signal += 1
        elif microprice < mid:
...
# =========================================================================
# GALAXY SOUNDS — from 557223 (intraday schedule + RV + pair-spread)
# =========================================================================
...
class GalaxySoundsTrader:
    ...
    PAIR_PARAMS = {
        ("SOLAR_FLAMES", "SOLAR_WINDS"): {
            "beta": -0.2788789274, "mean": 14003.382766, "std": 424.104697, "weight": 1.6,
        },
        ("DARK_MATTER", "PLANETARY_RINGS"): {
            "beta": 0.1875056763, "mean": 8207.849480, "std": 297.898660, "weight": 1.6,
        },
        ("DARK_MATTER", "SOLAR_WINDS"): {
            "beta": -0.0095836947, "mean": 10326.692052, "std": 330.659856, "weight": 0.8,
        },
        ("DARK_MATTER", "SOLAR_FLAMES"): {
            "beta": -0.0165546819, "mean": 10410.295812, "std": 330.616547, "weight": 0.8,
        },
        ("BLACK_HOLES", "SOLAR_FLAMES"): {
            "beta": -0.2493231700, "mean": 14232.507224, "std": 951.851252, "weight": 0.6,
        },
    }
    ...
    def run(self, state: TradingState):
        ...
        for s, score in scores.items():
            ...
            orders = self._orders_to_target(product, s, state.order_depths[product],
                                             pos, target, fair_skew[s], aggressive[s])
            if orders:
    ...
    def _orders_to_target(self, product, short_product, depth, pos, target,
                           fair_skew, aggressive) -> List[Order]:
        orders: List[Order] = []
        if not depth.buy_orders or not depth.sell_orders:
            return orders
        best_bid = max(depth.buy_orders.keys())
...
# =========================================================================
# PANEL / UV VISOR — from 559020 (selected-legs basket reversion)
# =========================================================================
...
class PanelTrader:
    ...
    def _snapshot(self, state: TradingState, symbols: List[str]):
        ...
        for symbol in symbols:
            ...
            snap[symbol] = {
                "bid": best_bid, "bid_vol": bid_vol,
                "ask": best_ask, "ask_vol": ask_vol, "mid": mid,
            }
        return snap

    def _add_orders(self, result, local_pos, trade_symbols, snap, side, trade_size,
                     exit_only=False) -> bool:
        qty = int(trade_size)
        if side == "buy":
    ...
    def _trade_strategy(self, state, result, local_pos, history, strat) -> None:
...
# =========================================================================
# SLEEP POD — from 566677 (intraday schedule + RV + pair-spread)
# =========================================================================
...
class SleepPodTrader:
    ...
    PAIR_PARAMS = {
        ("COTTON", "POLYESTER"): {
            "beta": 0.7948195677, "mean": 2116.504431, "std": 429.372024, "weight": 1.5,
        },
        ("POLYESTER", "SUEDE"): {
            "beta": 0.9337636163, "mean": 1198.064429, "std": 499.463608, "weight": 1.3,
        },
        ("LAMB_WOOL", "NYLON"): {
            "beta": 0.4005488291, "mean": 6841.563913, "std": 359.428666, "weight": 1.5,
        },
        ("LAMB_WOOL", "SUEDE"): {
            "beta": -0.0278286194, "mean": 11018.654332, "std": 412.866097, "weight": 0.7,
        },
        ("LAMB_WOOL", "POLYESTER"): {
            "beta": -0.0163054974, "mean": 10894.409467, "std": 413.014567, "weight": 0.7,
        },
    }
...
# =========================================================================
# Master Trader: dispatches to each sub-strategy with namespaced traderData
# =========================================================================
