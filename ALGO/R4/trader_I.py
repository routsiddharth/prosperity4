...
class Trader:
    ...
    # =====================================================
    # Counterparty pressure — pure ADDITIVE overlay (Agent I)
    # Layered on top of unmodified R3 fair-value.
    # No per-name priors; names are only group keys. Position is reconstructed
    # from the trade tape. We sweep CP_MULT to find the magnitude where this
    # additive nudge starts helping the baseline.
    # =====================================================
    CP_MULT = float(os.environ.get("CP_MULT", "0.10"))   # ticks per (signed pressure-flow unit)
    CP_LOOKBACK_TS = 2000
    CP_PRESSURE_EXTREME = 0.80
    CP_PRESSURE_MID     = 0.50
    CP_REVERSION_WEIGHT = 0.5
    CP_FLOW_CLIP        = 30.0
    CP_LRU_CAP          = 64
    ...
    def __init__(self):
        ...
        # Counterparty implied positions
        self.cp_pos = {}                # product -> name -> int (running implied position)
        self.cp_last_mt_ts = -1         # last market-trade ts already absorbed
    ...
    # =====================================================
    # HYDROGEL_PACK — v6 best  (R3 fair-value UNCHANGED, plus ADDITIVE CP overlay)
    # =====================================================
    def trade_hydrogel(self, product, order_depth, position):
        ...
        # ADDITIVE CP overlay (Agent I): R3 above is untouched.
        # cp_flow > 0 = bullish pressure → push fair_value UP → subtract a negative.
        cp_flow = self._pressure_flow(self.HYDROGEL)
        fair_value -= -cp_flow * self.CP_MULT
    ...
    # =====================================================
    # VELVETFRUIT_EXTRACT  (R3 fair-value UNCHANGED, plus ADDITIVE CP overlay)
    # =====================================================
    def trade_extract(self, product, order_depth, position):
        ...
        # ADDITIVE CP overlay (Agent I): R3 above is untouched.
        cp_flow = self._pressure_flow(self.EXTRACT)
        fair_value -= -cp_flow * self.CP_MULT
    ...
    # =====================================================
    # VEV_5300 — short premium  (UNCHANGED from baseline)
    # =====================================================
    ...
    # =====================================================
    # VEV_5400 — short premium (smaller clone of 5300)  (UNCHANGED)
    # =====================================================
    ...
    # =====================================================
    # VEV_5500 — short premium (UNCHANGED)
    # =====================================================
    ...
    # =====================================================
    # VEV_6000 / VEV_6500 — zero-bid settlement lottery (UNCHANGED)
    # =====================================================
    ...
    # =====================================================
    # Counterparty pressure helpers — name-agnostic, no priors.
    # =====================================================
    def _update_cp_positions(self, state):
        """Walk new market_trades, accumulate per-name net position per product."""
    ...
    def _pressure_flow(self, product):
        ...
        for t in state.market_trades.get(product, []):
            if now - t.timestamp > self.CP_LOOKBACK_TS:
                continue
