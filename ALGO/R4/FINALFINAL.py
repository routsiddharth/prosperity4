...
class Trader:
    ...
    # Worst-case kill-switch: stop trading HYDROGEL if mid drifts far from
    # BASE_FAIR for sustained ticks (regime change / asset redenomination).
    HYDROGEL_DRIFT_KILL_DISTANCE = 200
    HYDROGEL_DRIFT_KILL_TICKS    = 200

    # Regime re-classification window: 1% of the trading day.
    # During this window, accumulate bucket_premium samples; at the end,
    # re-classify the regime using the average. Position is preserved across
    # the switch — target is reset so the next tick re-evaluates under the
    # new thresholds.
    REGIME_RECLASSIFY_WINDOW_TS = 10_000
    ...
    def __init__(self):
        ...
        self.hydrogel_drift = {"oob_count": 0, "killed": False}
    ...
    def run(self, state: TradingState):
        ...
        for product in (self.V6000, self.V6500):
            if product in depths:
                orders[product] = self._trade_zero_lottery(
                    product, int(positions.get(product, 0))
                )
    ...
    def _trade_hydrogel(self, od: OrderDepth, position: int) -> List[Order]:
        ...
        if dist > self.HYDROGEL_DRIFT_KILL_DISTANCE:
            self.hydrogel_drift["oob_count"] += 1
            if self.hydrogel_drift["oob_count"] >= self.HYDROGEL_DRIFT_KILL_TICKS:
                self.hydrogel_drift["killed"] = True
                return orders
        else:
            self.hydrogel_drift["oob_count"] = 0
    ...
    def _update_regime(self, product: str, cfg: dict, timestamp: int, bucket_premium: float):
        ...
        if ps["current_day_id"] is None or day_id != ps["current_day_id"]:
            ...
            ps["target"]                  = 0
            ps["regime_committed"]        = False
            ps["premium_sum"]             = 0.0
            ps["premium_count"]           = 0
            self._classify_regime(ps, cfg, bucket_premium)

        if not ps["regime_committed"]:
            ps["premium_sum"]   += bucket_premium
            ps["premium_count"] += 1

            time_in_day = timestamp % self.DAY_TS
            if time_in_day >= self.REGIME_RECLASSIFY_WINDOW_TS:
                avg = ps["premium_sum"] / ps["premium_count"]
                old_low     = ps["low_threshold"]
                old_high    = ps["high_threshold"]
                old_flatten = ps["flatten_after_ts"]
                self._classify_regime(ps, cfg, avg)
                if (ps["low_threshold"]    != old_low    or
                    ps["high_threshold"]   != old_high   or
                    ps["flatten_after_ts"] != old_flatten):
                    ps["target"] = 0
                ps["regime_committed"] = True

    def _classify_regime(self, ps: dict, cfg: dict, premium: float):
        if premium >= cfg["high_open"]:
            ps["low_threshold"]    = cfg["high_low"]
            ps["high_threshold"]   = cfg["high_high"]
            ps["flatten_after_ts"] = cfg["high_flatten"]
        elif premium >= cfg["mid_open"]:
    ...
    def _trade_zero_lottery(self, product: str, position: int) -> List[Order]:
        limit = self.LIMITS[product]
        remaining = limit - position
        if remaining > 0:
            return [Order(product, 0, remaining)]
        return []
    ...
    def _restore(self, trader_data: str):
        ...
        for product, ps in saved.get("state", {}).items():
            ...
            s["regime_committed"]       = bool(ps.get("regime_committed", False))
            s["premium_sum"]            = float(ps.get("premium_sum", 0.0))
            s["premium_count"]          = int(ps.get("premium_count", 0))

        hd = saved.get("hydrogel_drift", {})
        self.hydrogel_drift["oob_count"] = int(hd.get("oob_count", 0))
        self.hydrogel_drift["killed"]    = bool(hd.get("killed", False))

    def _serialize(self) -> str:
        return json.dumps(
            {"state": self.state, "hydrogel_drift": self.hydrogel_drift},
            separators=(",", ":"),
        )