...
class Trader:
    ...
    def _should_dampen_short_take(self, product, market_trades):
        """Return True if we should skip an aggressive short-premium TAKE
        because the dominant bid-side counterparty in this product is a
        flagged 'directional seller' (systematic OTM seller, historically
        profitable since vouchers expired OTM in our sample)."""
        if product not in self.CP_FILTER_PRODUCTS:
            return False
        name = self._bid_side_dominant_counterparty(product, market_trades)
        if not name:
            return False
        direction = self._cp_directional(product, name)
        # Dampen ONLY when the counterparty has lopsided NET-SELL flow.
        # Rationale: in our observed data, sellers of OTM vouchers have
        # been systematically right (vouchers stayed OTM). If we'd be
        # selling INTO a bid placed by such a counterparty, we'd be
        # joining a side that likely contributes adverse selection.
        # NOTE: this filter only ever reduces activity — it cannot grow
        # positions or bypass premium thresholds.
    ...
    def run(self, state: TradingState):
        ...
        if self.OPTION in state.order_depths:
            ...
            if self.halted.get(self.OPTION):
                ...
                damp = self._should_dampen_short_take(self.OPTION, state.market_trades)
                orders_by_product[self.OPTION] = self.trade_5300(
                    option_depth, option_position, underlying_mid, imbalance, damp
                )
        ...
        if self.V5400 in state.order_depths:
            ...
            if self.halted.get(self.V5400):
                ...
                damp = self._should_dampen_short_take(self.V5400, state.market_trades)
                orders_by_product[self.V5400] = self.trade_5400(
                    v5400_depth, v5400_position, underlying_mid, imbalance, damp
                )
        ...
        if self.V5500 in state.order_depths:
            ...
            if self.halted.get(self.V5500):
                ...
                damp = self._should_dampen_short_take(self.V5500, state.market_trades)
                orders_by_product[self.V5500] = self.trade_5500(
                    v5500_depth, v5500_position, underlying_mid, imbalance, damp
                )
    ...
    def trade_5300(self, order_depth, position, underlying_mid, imbalance, cp_damp=False):
        product = self.OPTION
        orders = []
        ...
        if position > self.MAX_SHORT_5300:
            ...
            if premium >= self.SELL_THRESHOLD and imbalance <= 0:
                ...
                # SAFETY DAMP: cut size in half when counterparty filter fires
                if cp_damp:
    ...
    def trade_5400(self, order_depth, position, underlying_mid, imbalance, cp_damp=False):
        product = self.V5400
        orders = []
    ...
    def trade_5500(self, order_depth, position, underlying_mid, imbalance, cp_damp=False):
        product = self.V5500
        orders = []
