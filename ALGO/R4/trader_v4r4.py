...
class Trader:
    ...
    # =====================================================
    # Counterparty inventory-pressure (H signal) — name-agnostic
    # Uses Trade.buyer / Trade.seller only as group keys.
    # No per-name priors. Position is reconstructed from the trade tape.
    # =====================================================
    CP_W = float(os.environ.get("CP_W", "0.50"))   # convex weight on CP signal in [0,1]
    CP_LOOKBACK_TS = 2000
    CP_PRESSURE_EXTREME = 0.80
    CP_PRESSURE_MID     = 0.50
    CP_REVERSION_WEIGHT = 0.5     # forced-flatten signal weighted vs continuation
    HYDROGEL_CP_SCALE   = 0.40    # ticks per (signed pressure-flow unit), HYDROGEL
    EXTRACT_CP_SCALE    = 0.30    # ticks per (signed pressure-flow unit), EXTRACT
    CP_FLOW_CLIP        = 30.0    # clip aggregate flow magnitude
    CP_LRU_CAP          = 64      # max tracked names per product

    # G overlay — voucher-side streak / concentration gate
    CP_VG_LOOKBACK_TS  = 5000
    CP_VG_STREAK_MIN   = 4
    CP_VG_HERF_MIN     = 0.25
    CP_VG_THRESH_BUMP  = 1.0      # premium ticks added to SELL_THRESHOLD when gate trips
    ...
    def __init__(self):
        ...
        self.cp_pos = {}                # product -> name -> int (running implied position)
        self.cp_last_mt_ts = -1         # last market-trade ts already absorbed into cp_pos

    def run(self, state: TradingState):
        self._restore_state(state.traderData)
        self._update_cash_from_trades(state)
        self._update_cp_positions(state)
        self._cur_state = state                           # for helpers below
        self._voucher_bump = self._compute_voucher_bump(state)
    ...
    def trade_hydrogel(self, product, order_depth, position):
        ...
        # Inventory management — always at full strength (risk management, not a market read)
        fair_value -= position_skew * self.HYDROGEL_INVENTORY_SKEW
        fair_value -= self.HYDROGEL_INVENTORY_SKEW_QUAD * (position_skew ** 3)
        # Blend R3 book-imbalance read with counterparty-pressure read
        r3_signal = imbalance * self.HYDROGEL_IMBALANCE_SKEW
        cp_flow   = self._pressure_flow(self.HYDROGEL, self.CP_LOOKBACK_TS)
        cp_signal = -cp_flow * self.HYDROGEL_CP_SCALE
        fair_value -= (1.0 - self.CP_W) * r3_signal + self.CP_W * cp_signal
    ...
    def trade_extract(self, product, order_depth, position):
        ...
        # Inventory management — always at full strength
        fair_value -= position_skew * self.EXTRACT_INVENTORY_SKEW
        # Blend R3 book-imbalance with counterparty-pressure
        r3_signal = imbalance * self.EXTRACT_IMBALANCE_SKEW
        cp_flow   = self._pressure_flow(self.EXTRACT, self.CP_LOOKBACK_TS)
        cp_signal = -cp_flow * self.EXTRACT_CP_SCALE
        fair_value -= (1.0 - self.CP_W) * r3_signal + self.CP_W * cp_signal
    ...
    def trade_5300(self, order_depth, position, underlying_mid, imbalance):
        ...
        if position > self.MAX_SHORT_5300:
            ...
            sell_thr   = self.SELL_THRESHOLD        + self.CP_W * self._voucher_bump
            strong_thr = self.STRONG_SELL_THRESHOLD + self.CP_W * self._voucher_bump

            if premium >= sell_thr and imbalance <= 0:
                sell_size = self.NORMAL_SELL_SIZE

                if premium >= strong_thr:
            ...
            elif premium >= strong_thr and imbalance <= 0.15:
                qty = min(
    ...
    def trade_5400(self, order_depth, position, underlying_mid, imbalance):
        ...
        if position > self.V5400_MAX_SHORT:
            ...
            sell_thr   = self.V5400_SELL_THRESHOLD        + self.CP_W * self._voucher_bump
            strong_thr = self.V5400_STRONG_SELL_THRESHOLD + self.CP_W * self._voucher_bump
    ...
    def trade_5500(self, order_depth, position, underlying_mid, imbalance):
        ...
        if position > self.V5500_MAX_SHORT:
            ...
            sell_thr   = self.V5500_SELL_THRESHOLD        + self.CP_W * self._voucher_bump
            strong_thr = self.V5500_STRONG_SELL_THRESHOLD + self.CP_W * self._voucher_bump
    ...
    def _restore_state(self, trader_data):
        ...
        self.cp_pos = d.get("cp_pos", {})
        self.cp_last_mt_ts = d.get("cp_last_mt_ts", -1)
    ...
    def _serialize_state(self):
        return json.dumps({
            ...
            "cp_pos": self.cp_pos,
            "cp_last_mt_ts": self.cp_last_mt_ts,
        })

    # =====================================================
    # Counterparty pressure helpers (H signal) — name-agnostic
    # =====================================================
    def _update_cp_positions(self, state):
        """Walk market_trades since last seen, accumulate per-name net position per product."""
        latest = self.cp_last_mt_ts
        for product, trades in state.market_trades.items():
            d = self.cp_pos.setdefault(product, {})
            for t in trades:
                if t.timestamp <= self.cp_last_mt_ts:
                    continue
                if t.buyer:
                    d[t.buyer]  = d.get(t.buyer, 0)  + t.quantity
                if t.seller:
                    d[t.seller] = d.get(t.seller, 0) - t.quantity
                if t.timestamp > latest:
                    latest = t.timestamp
        self.cp_last_mt_ts = latest
        # LRU-ish cap: keep top-N by |pos| per product
        for p, names in self.cp_pos.items():
            if len(names) > self.CP_LRU_CAP:
                top = sorted(names.items(), key=lambda kv: -abs(kv[1]))[:self.CP_LRU_CAP]
                self.cp_pos[p] = dict(top)

    def _pressure_flow(self, product, lookback_ts):
        """Aggregate signed pressure-weighted flow over recent trades.
        Positive = bullish (extreme accumulators on long side, or forced shorts covering).
        Negative = bearish.
        """
        state = self._cur_state
        now = state.timestamp
        cp_pos_p = self.cp_pos.get(product, {})
        limit = self.LIMITS.get(product, 200)
        flow = 0.0
        for t in state.market_trades.get(product, []):
            if now - t.timestamp > lookback_ts:
                continue
            for name, sign in ((t.buyer, +1), (t.seller, -1)):
                if not name:
                    continue
                pos = cp_pos_p.get(name, 0)
                pressure = max(-1.0, min(1.0, pos / limit))
                ap = abs(pressure)
                if ap < self.CP_PRESSURE_MID:
                    continue
                same_side = (sign > 0 and pressure > 0) or (sign < 0 and pressure < 0)
                if ap >= self.CP_PRESSURE_EXTREME and same_side:
                    # Informed accumulator → continuation
                    flow += sign * ap * t.quantity
                elif (not same_side) and self.CP_PRESSURE_MID <= ap < self.CP_PRESSURE_EXTREME:
                    # Forced flatten → reversion (price moves AGAINST the trade direction)
                    flow += -sign * ap * t.quantity * self.CP_REVERSION_WEIGHT
        # Clip to keep convex-combination magnitude sane
        if flow >  self.CP_FLOW_CLIP: flow =  self.CP_FLOW_CLIP
        if flow < -self.CP_FLOW_CLIP: flow = -self.CP_FLOW_CLIP
        return flow

    def _compute_voucher_bump(self, state):
        """G overlay — return premium-threshold bump when one counterparty dominates EXTRACT bull side."""
        now = state.timestamp
        trades = state.market_trades.get(self.EXTRACT, [])
        recent = [t for t in trades if now - t.timestamp <= self.CP_VG_LOOKBACK_TS]
        if not recent:
            return 0.0
        # Buy-side streak by any single counterparty
        recent_sorted = sorted(recent, key=lambda t: t.timestamp)
        max_streak = 0
        cur_buyer = None
        cur_run = 0
        for t in recent_sorted:
            if t.buyer == cur_buyer and t.buyer:
                cur_run += 1
            else:
                cur_buyer = t.buyer
                cur_run = 1
            if cur_run > max_streak:
                max_streak = cur_run
        # Herfindahl on buyer volumes (bull side concentration)
        from collections import defaultdict
        vol = defaultdict(int)
        tot = 0
        for t in recent:
            if t.buyer:
                vol[t.buyer] += t.quantity
                tot += t.quantity
        if tot <= 0:
            return 0.0
        herf = sum((v / tot) ** 2 for v in vol.values())
        if max_streak >= self.CP_VG_STREAK_MIN and herf >= self.CP_VG_HERF_MIN:
            return self.CP_VG_THRESH_BUMP
        return 0.0
