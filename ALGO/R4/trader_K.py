...
class Trader:
    ...
    # =====================================================
    # Voucher-Only Counterparty Gate (Agent K)
    # ---------------------------------------------------------------
    # Compute, every tick, on EXTRACT market_trades over the last
    # K_LOOKBACK ticks: top_buyer_share and that buyer's trade count.
    # If top_buyer_share > K_SHARE_THRESHOLD AND its trade count >=
    # K_MIN_COUNT, then bump VEV_5300/5400/5500 SELL_THRESHOLD and
    # STRONG_SELL_THRESHOLD by K_BUMP_TICKS.
    # Symmetric (optional): if top_seller_share dominates, lower the
    # COVER_THRESHOLD by K_COVER_DROP so we cover shorts faster.
    # NO offline per-name priors — names are group keys only.
    # =====================================================
    K_SHARE_THRESHOLD = float(os.environ.get("SHARE_THRESHOLD", "0.55"))
    K_MIN_COUNT       = int(os.environ.get("MIN_COUNT", "3"))
    K_BUMP_TICKS      = float(os.environ.get("BUMP_TICKS", "2"))
    K_LOOKBACK        = int(os.environ.get("LOOKBACK", "1000"))
    K_COVER_DROP      = float(os.environ.get("COVER_DROP", "0"))  # 0 = symmetric branch off
    ...
    def __init__(self):
        ...
        # Voucher-Only Counterparty Gate (Agent K) — rolling buffer of EXTRACT
        # market trades over the last K_LOOKBACK ticks. Each entry:
        # (timestamp, buyer, seller, quantity). In-memory only (per-day instance).
        self._k_extract_buf = []
        self._k_seen_keys = set()
    ...
    def run(self, state: TradingState):
        ...
        # Voucher-Only Counterparty Gate (Agent K).
        # Maintain a rolling per-day buffer of EXTRACT market trades, then
        # bump SELL/STRONG_SELL thresholds on VEV_5300/5400/5500 when one
        # counterparty dominates EXTRACT buy flow over the lookback window.
        self._k_update_extract_buffer(state)
        v_bump, v_cover_drop = self.compute_voucher_gate(state)
    ...
    def _k_update_extract_buffer(self, state):
        """Append this tick's EXTRACT market_trades to a rolling buffer,
        then drop entries older than K_LOOKBACK ticks. Dedupe by
        (ts, buyer, seller, qty, price)."""
        now = state.timestamp
        window_ts = self.K_LOOKBACK * 100
        trades = state.market_trades.get(self.EXTRACT, [])
        for t in trades:
            key = (t.timestamp, t.buyer, t.seller, t.quantity, t.price)
            if key in self._k_seen_keys:
                continue
            self._k_seen_keys.add(key)
            self._k_extract_buf.append((t.timestamp, t.buyer, t.seller, t.quantity))
        # Evict expired
        cutoff = now - window_ts
        if self._k_extract_buf and self._k_extract_buf[0][0] < cutoff:
            self._k_extract_buf = [
                e for e in self._k_extract_buf if e[0] >= cutoff
            ]
            # Rebuild seen-keys (memory tradeoff; bounded by buffer size)
            # Note: original key has price; rebuild with same composite is awkward,
            # so just rebuild seen_keys as the smaller set of buffered ones.
            # Acceptable: a stale entry matching an evicted key won't reappear because
            # market_trades only contains the *current* tick.
            self._k_seen_keys = {
                (e[0], e[1], e[2], e[3]) for e in self._k_extract_buf
            }
            # (We intentionally allow re-add for trades not in buffer — the
            # current-tick-only nature of state.market_trades makes collisions
            # impossible across ticks.)

    def compute_voucher_gate(self, state):
        """Voucher-Only Gate (Agent K).

        Scan rolling buffer of EXTRACT market_trades over the last K_LOOKBACK
        ticks. Compute, per side:
            top_share = (volume of top counterparty) / total volume
            top_count = trade count for that top counterparty
        If buy-side top_share > K_SHARE_THRESHOLD AND top_count >= K_MIN_COUNT,
        return (vbump = K_BUMP_TICKS, vcover_drop = 0).
        If sell-side same condition, return (0, K_COVER_DROP).
        Names are group keys only — no per-name priors.
        """
        if not self._k_extract_buf:
            return 0.0, 0.0
        ...
        tot_buy = 0
        tot_sell = 0
        for ts, buyer, seller, qty in self._k_extract_buf:
            if buyer:
                buy_vol[buyer] = buy_vol.get(buyer, 0) + qty
                buy_cnt[buyer] = buy_cnt.get(buyer, 0) + 1
                tot_buy += qty
            if seller:
                sell_vol[seller] = sell_vol.get(seller, 0) + qty
                sell_cnt[seller] = sell_cnt.get(seller, 0) + 1
                tot_sell += qty

        vbump = 0.0
        vcover_drop = 0.0

        if tot_buy > 0 and buy_vol:
            top_buyer = max(buy_vol, key=buy_vol.get)
            top_share = buy_vol[top_buyer] / tot_buy
            top_count = buy_cnt[top_buyer]
            if top_share > self.K_SHARE_THRESHOLD and top_count >= self.K_MIN_COUNT:
                vbump = self.K_BUMP_TICKS

        if self.K_COVER_DROP > 0 and tot_sell > 0 and sell_vol:
            top_seller = max(sell_vol, key=sell_vol.get)
            top_share = sell_vol[top_seller] / tot_sell
            top_count = sell_cnt[top_seller]
            if top_share > self.K_SHARE_THRESHOLD and top_count >= self.K_MIN_COUNT:
                vcover_drop = self.K_COVER_DROP

        return vbump, vcover_drop
    ...
    def trade_5300(self, order_depth, position, underlying_mid, imbalance,
        ...
        sell_thr   = self.SELL_THRESHOLD + v_bump
        strong_thr = self.STRONG_SELL_THRESHOLD + v_bump
        cover_thr  = self.COVER_THRESHOLD - v_cover_drop

        if position < 0 and premium <= cover_thr:
    ...
    def trade_5400(self, order_depth, position, underlying_mid, imbalance,
        ...
        sell_thr   = self.V5400_SELL_THRESHOLD + v_bump
        strong_thr = self.V5400_STRONG_SELL_THRESHOLD + v_bump
        cover_thr  = self.V5400_COVER_THRESHOLD - v_cover_drop
    ...
    def trade_5500(self, order_depth, position, underlying_mid, imbalance,
        ...
        sell_thr   = self.V5500_SELL_THRESHOLD + v_bump
        strong_thr = self.V5500_STRONG_SELL_THRESHOLD + v_bump
        cover_thr  = self.V5500_COVER_THRESHOLD - v_cover_drop
